...
//   판독 우선순위:
//     1) 강한 공공 오버라이드(시청·도청·정부부처·청·선거관리위원회 등) → 관공서
//     2) 회사 법인격 마커((주)/㈜/주식회사 등) → 민간
//     3) 명시적 민간 키워드(대학교/병원/재단법인/(재)/이엔씨/이앤씨/전자/재개발/위원회/조합 등) → 민간
//     4) 행정구역(시/군/구/도/읍/면/동/리) 토큰 → 관공서
//     5) 일반 공공 키워드(공사/공단/공기업/연구원 등, 단 회사형 접미 제외) → 관공서
//     6) 2~4자 한글 단독 → 민간 (개인 이름 추정)
//     7) 기본값 → 민간
export const isPrivateClient = (clientName?: string | null): boolean => {
  ...
  // 1) 강한 공공 오버라이드 — 회사 마커보다 먼저
  if (/시청|도청|군청|구청|읍사무소|면사무소|동사무소|주민센터|행정복지센터|환경부|국토교통부|산업통상자원부|보건복지부|농림축산식품부|해양수산부|문화체육관광부|교육부|과학기술정보통신부|외교부|통일부|법무부|국방부|행정안전부|기획재정부|여성가족부|중소벤처기업부|국세청|관세청|특허청|병무청|조달청|통계청|검찰청|감사원|국회|법원|선거관리위원회|국가정보원|경찰청|소방청|기상청|문화재청|산림청|농촌진흥청|재정관리단|수도사업소|종합건설사업소|건설사업소|상수도사업소|하수도사업소|치수사업소|도로사업소|건설관리사업소|도시관리사업소|시설관리사업소|토지관리사업소|환경사업소|국군|군수사령부|방위사업청|병참|보충대|훈련소|군병원|군악대|국방시설|국방과학|군수지원|기지사령부|함대|비행단/.test(name)) return false;
  // 한국토지주택공사/수자원공사/도로공사/철도공사/공항공사 등 명확한 공기업
  if (/한국토지주택|수자원공사|도로공사|철도공사|공항공사|한국전력|한국가스|한국수력|한국철도/.test(name)) return false;

  // 2) 회사 법인격 마커 → 무조건 민간
  if (/\(주\)|\(유\)|\(합\)|\(재\)|\(사\)|\(학\)|\(의\)|㈜|㈔|㈐|㈒|㈕|주식회사|유한회사|유한책임회사|합자회사|합명회사|협동조합|영농조합법인|영어조합법인/.test(name)) return true;
  if (/Co\.,?Ltd|Corporation|Corp\.|Inc\.|LLC|Ltd\./i.test(name)) return true;

  // 3) 명시적 민간 키워드 (확장)
  //   - 학교/의료/아파트/조합/엔지니어링/법인 등
  //   - 회사형 접미: 이엔씨/이앤씨/ENC/E&C/전자/전기/산업/건설/개발/기술/엔지니어링/디자인/소프트/시스템/네트웍/네트워크/텔레콤/물산/상사/홀딩스/그룹/파트너스/컨소시엄
  //   - 재단법인/(재)/사단법인/(사)/학교법인/의료법인
  //   - 재개발/재건축/조합/위원회(공공 위원회는 위 1단계에서 이미 처리)
  if (/대학교|대학원|대학|학원|병원|의원|치과|한의원|요양원|요양병원|아파트|입주자대표|재건축|재개발|조합|엔지니어링|이엔씨|이앤씨|건축사사무소|건축사|기술사사무소|컨설팅|컨설턴트|로펌|법무법인|회계법인|세무법인|특허법인|감정평가법인|연합회|협회|단체|재단법인|사단법인|학교법인|의료법인|위원회/.test(name)) return true;
  if (/E&C|ENC/i.test(name)) return true;
  // 회사형 접미사로 끝나는 경우 (한글 2자 이상 + 접미)
  if (/[가-힣A-Za-z0-9]{2,}(이엔씨|이앤씨|전자|전기|산업|건설|개발|기술|디자인|소프트|시스템|네트웍|네트워크|텔레콤|물산|상사|홀딩스|그룹|파트너스|컴퍼니|컴퍼니스|코퍼레이션)$/.test(name)) return true;
  ...
  if (/^[가-힣]{2,8}(시청|도청|군청|구청)$/.test(name)) return false;
  if (/^[가-힣]{2,8}(시|군|구|도)$/.test(name)) return false;

  // 5) 일반 공공 키워드
  //   주의: "공사"는 명사 끝일 때만 공기업으로 본다. "기술개발공사"처럼 회사명에 포함된 케이스를 줄이기 위해
  //   "공사"는 토큰 단독 또는 명확한 공기업 패턴(앞 3단계)에서 처리하고, 여기서는 제외한다.
  if (/청사|국토관리|환경관리|환경청|국립|공립|공단|본부|관리소|관리공단|관리원|연구원|연구소|진흥원|진흥공단|개발원|군부대|사단|여단|부대|정부|교육청|교육지원청|보건소|소방서/.test(name)) return false;
